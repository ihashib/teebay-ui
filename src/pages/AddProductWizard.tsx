...
import { Button, Box, Title, TextInput, Textarea, NumberInput, Group, Text, MultiSelect, Select } from '@mantine/core';
...
export default function AddProductWizard() {
  ...
  const [touched, setTouched] = useState({} as Partial<Record<keyof FormData, boolean>>);

  const errors: Partial<Record<keyof FormData, string>> = {};
  if (currentStep === 0 && touched.title && formData.title.trim() === '') {
    errors.title = 'Product title is required';
  }
  if (currentStep === 1 && touched.categories && formData.categories.length === 0) {
    errors.categories = 'Select at least one category';
  }
  if (currentStep === 2 && touched.description && formData.description.trim() === '') {
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 3:
        return formData.price > 0 && formData.rentPrice > 0 && formData.rentUnit !== '';
      default:
        return true;
    }
  };
  ...
  const handleNext = () => {
    setTouched(prev => ({ ...prev, [steps[currentStep].component]: true }));
    if (!isStepValid()) {
      return; 
    }
    setTouched({});
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data } = await createProduct({ variables: { input: formData } });
        ...
        {currentStep === 3 && (
           <>
            <NumberInput
              ...
              onChange={value => handleChange('price', typeof value === 'string' ? parseFloat(value) || 0 : value || 0)}
            ...
            <NumberInput
              ...
              onChange={value => handleChange('rentPrice', typeof value === 'string' ? parseFloat(value) || 0 : value || 0)}
